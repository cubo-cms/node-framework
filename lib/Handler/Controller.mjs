    ...
    delete: function() {
      return new Promise((resolve, reject) => {
        ...
        Namespace.JLoader.load(this.get('dataSource')).then((dataSource) => {
          if(this.get('user') == 'nobody') {
            reject(Namespace.JResponse.respond('unauthenticated', { message: `This method requires authentication` }, this));
          } else if(dataSource.readOnly || dataSource.systemOnly) {
            reject(Namespace.JResponse.respond('forbidden', { message: `This method is forbidden for ${this.get('user')}` }, this));
          } else {
            // Resolve data
            resolve({ dataSource: dataSource[this.get('dataType')] });
          }
        }).catch((error) => {
          reject(error);
        });
