...
export default class Model extends Handler {
    ...
    get: function() {
      return new Promise((resolve, reject) => {
        ...
        Namespace.Driver.create(this.prepareData(), this).then((driver) => {
          ...
          _log_.info({ message: `${this.type} returns success`, source: this.name, payload: response });
          // Resolve response
          resolve(response);
        }).catch((error) => {
          _log_.info({ message: `${this.type} returns failure`, source: this.name, payload: error });
          // Reject with error
          reject(error);
        });
      });
    ...
    return new Promise((resolve, reject) => {
      ...
      if(method) {
        if(typeof this.method[method] === 'function') {
          _log_.info({ message: `${this.type} invokes method \"${method}\"`, source: this.type });
          method = this.method[method].bind(this);
          resolve(method());
        } else {
          _log_.warning({ message: `${this.type} fails to invoke method \"${method}\"`, source: this.type });
          reject(Namespace.JResponse.respond('notAllowed', { header: { Allow: Object.keys(this.method).join(', ').toUpperCase() } }, this.type));
        }
      } else {
        _log_.warning({ message: `${this.type} cannot determine method`, source: this.type });
        reject(Namespace.JResponse.respond('notAllowed', { header: { Allow: Object.keys(this.method).join(', ').toUpperCase() } }, this.type));
      }
    });
  }
