  ...
  static loadFromURI(data = '') {
    ...
    return new Promise((resolve, reject) => {
      if(!data.startsWith('http')) {
        reject(Namespace.JResponse.error({ message: `Unacceptable protocol` }, this.name));
      } else {
        let uri = parse(data, true);
        const session = http2.connect(uri.protocol + '//' + uri.host).on('error', (error) => {
          reject(Namespace.JResponse.error({ message: `Failed to connect to URI`, payload: error }, this.name));
        });
        ...
        request.on('data', (chunk) => {
          ...
          reject(Namespace.JResponse.error({ message: `Failed to read data`, payload: error }, this.name));
          session.close();
        });
      }
    });
