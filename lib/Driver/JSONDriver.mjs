...
import Driver from '../Class/Driver.mjs';
      ...
      Class.load(this.get('uri'))
        .then((data) => {
          ...
          if(this.get('id')) {
            ...
            if(result) {
              resolve(JResponse.success({ data: result, dataType: this.get('dataType'), id: this.get('id') }));
            } else {
              reject(JResponse.respond('notFound'));
            }
          } else {
            ...
            if(result) {
              ...
              resolve(JResponse.success({ data: result, dataType: this.get('dataType'), size: result.length, page: this.get('page'), pageSize: this.get('pageSize'), totalSize: total }));
            } else {
              reject(JResponse.respond('badRequest'));
            }
          }
        }).catch((error) => {
          reject(JResponse.error({ message: error }));
        });
    });
  }
}
