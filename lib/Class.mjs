...
export default class Class {
  /** @static @function create(data,caller,dataType)
    ...
    **/
  static create(data = {}, caller = undefined, dataType = '') {
    return new Promise((resolve, reject) => {
      this.load(data).then((data) => {
        let instance;
        dataType = data.dataType ?? dataType;
        if(this.exists(data.dataType + this.name)) {
          instance = new Namespace[data.dataType + this.name](data);
          if(caller && global.log) log.debug({ message: `${caller.name} creates ${data.dataType + this.name} instance`, source: this.name, payload: data });
        } else {
  ...
  /** @static @function exists(module,dataType)
    ...
    * @return {boolean}
    **/
  static exists(module, dataType = '') {
    return Namespace.exists(dataType + module);
  }
  ...
  load(data = {}) {
    return new Promise((resolve, reject) => {
      Namespace.JLoader.load(this.#dataSource).then((thisData) => {
