  ...
  static load(data = this.#registry) {
    return new Promise((resolve, reject) => {
      ...
      Promise.allSettled(promises)
        .then(() => {
          Log.success({ message: `Namespace completed loading modules`, source: this.name, payload: this.#succeeded });
  ...
  static register(searchPath = this.default.searchPath, basePath = JLoader.basePath) {
    return new Promise((resolve, reject) => {
      this.registerPath(JLoader.resolvePath(searchPath))
        .then((namespace) => {
          Log.success({ message: `Namespace completed registering modules`, source: this.name, payload: this.#registry });
