...
export default class Application extends Class {
  ...
  invokeController(data) {
    ...
    return new Promise((resolve, reject) => {
      ...
      _log_.info({ message: `Error generated`, source: this.type, payload: errorResponse });
  ...
  invokeMethod(controller) {
    _log_.info({ message: `Application invokes Controller method`, source: this.type });
    return new Promise((resolve, reject) => {
  ...
  server(data = this.data.server) {
    // Apply log settings
    if(!global._log_) global._log_ = new Log(this.get('log'));
    // Start the server
