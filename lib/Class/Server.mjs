...
export default class Server extends Class {
  ...
  invokeProcessor(request, response, router) {
    Log.info({ message: `Server receives request`, source: this.type, payload: `${request.method} ${request.url}` });
    let payload = [];
  ...
  listen(router, port = undefined) {
    ...
    const server = http2.createSecureServer({
      ...
      Log.success({ message: `Server listens on port ${port}`, source: this.type });
    });
  }
