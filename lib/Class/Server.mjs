/** Server module starts the HTTP/2 server to listen for requests. The requests
  * are passed back to the application.
  *
  * @package        @cubo-cms/node-framework
  * @module         /lib/Class/Server.mjs
  * @version        0.5.40
  * @copyright      2021 Cubo CMS <https://cubo-cms.com/COPYRIGHT.md>
  * @license        ISC license <https://cubo-cms.com/LICENSE.md>
  * @author         Papiando <info@papiando.com>
  **/

import fs from 'fs';
import http2 from 'http2';

import Namespace from '../../Namespace.mjs';

export default class Server extends Namespace.Class {
  static default = {
    digest: 'sha-256',
    poweredBy: 'Cubo CMS',
    secure: true,
    cert: `-----BEGIN CERTIFICATE-----

zkYQmQOc+omJCB8jzFrVdJfb98YeY1F6G81ZPW6iu2OGWDLf
-----END CERTIFICATE-----`,
  key: `-----BEGIN PRIVATE KEY-----

VMU03PVNxCQuXJQ3kKdK4ovH
-----END PRIVATE KEY-----`
  };

  /** @property {object} default - holds default values
    **/
  default = {
    port: 8443
  };

  request(data = {}, request, response) {
    let payload = [];
    let output;
    let requestReceived = Date.now();
    request.on('data', (chunk) => {
      // Get request data and store as payload
      payload.push(chunk);
    }).on('end', () => {
      payload = Buffer.concat(payload).toString();
      // Invoke execution of the application handler
      let appData = { payload: payload, requestReceived: requestReceived, sessionId: Namespace.Cookie.unserialize(request.headers['cookie']).sessionId };
      this.caller.invoke(request, response, appData).then((data) => {
        if(this.has('poweredBy')) response.setHeader('X-Powered-By', this.get('poweredBy'));
        if(data.status == 'success') {
          output = JSON.stringify(data.data);
        } else {
          output = JSON.stringify(data);
        }
        response.statusCode = data.statusCode || 500;
        response.setHeader('Content-Length', output.length);
        response.end(output);
      });
    }).on('error', (error) => {
      throw error;
    });
  }
  listen(data = {}) {
    return new Promise((resolve, reject) => {
      // Load data
      this.load(data).then((data) => {
        return Namespace.Loader.loadAll({ cert: this.get('cert'), key: this.get('key') });
      }).then((certData) => {
        this.data = {...this.data, ...certData};
        let server;
        // Call server to listen for requests
        if(this.get('secure')) {
          // Secure connection
          server = http2.createSecureServer({ cert: this.get('cert'), key: this.get('key'), allowHTTP1: this.get('allowHTTP1') });
        } else {
          // Although there is an option to use an insecure connection, several browsers will not accept these connections
          server = http2.createServer();
        };
        server.on('request', (request, response) => {
          this.request(this.data, request, response);
        }).on('error', (error) => {
          throw error;
        }).listen(this.get('port'), (error) => {
          if(error) throw error;
          log.info(`Server is listening on port ${this.get('port')}`);
        });
        resolve(`Server starts`);
      }).catch((error) => {
        reject(error);
      });
    })
  }
}
