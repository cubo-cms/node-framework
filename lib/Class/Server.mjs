...
export default class Server extends Namespace.Class {
  ...
  invokeProcessor(request, response, router) {
    ...
    request.on('data', (chunk) => {
      ...
      // Add remote address to headers
      request.headers['remote-address'] = request.connection.remoteAddress;
      ...
      router({ headers: request.headers, method: request.method, url: request.url, payload: payload, contentType: request.headers['content-type'], timestamp: Date.now() }).then((result) => {
        ...
        if(this.get('poweredBy')) response.setHeader('X-Powered-By', this.get('poweredBy'));
        ...
        if(this.get('digest')) response.setHeader('Digest', Namespace.Session.hash(output, this.get('digest')));
        response.end(output);
      }).catch((error) => {
        _log_.error(error);
