...
export default class Router extends Namespace.Class {
  ...
  parse(request) {
    return new Promise((resolve, reject) => {
      ...
      if(foundRoute && parsedData.method != 'skip') {
        ...
        data.remoteAddress = request.remoteAddress;
        Object.assign(data, parsedData);
        _log_.debug({ message: `Router returns results`, source: this.type, payload: data });
        resolve(Namespace.JResponse.success({ data: data }, this.type));
      } else {
        reject(Namespace.JResponse.respond('notFound', { message: 'Router ignores this route' }, this.type));
      }
    });
  }
}
