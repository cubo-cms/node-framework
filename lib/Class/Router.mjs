...
export default class Router extends Class {
  ...
  parse(request) {
    return new Promise((resolve, reject) => {
      Log.info({ message: `Router parses request`, source: this.type, payload: `${request.method} ${request.url}` });
      ...
      if(foundRoute && parsedData.method != 'skip') {
        ...
        Log.debug({ message: `Router returns results`, source: this.type, payload: data });
        resolve(JResponse.success({ data: data }));
      } else {
        reject(JResponse.respond('notAllowed', { message: 'Router ignores this route', type: 'warning', source: this.type }));
      }
    });
  }
}
