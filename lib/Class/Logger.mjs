...
export default class Logger extends Class {
  ...
  log(data, status = 'debug') {
    if(!this.get('suppressStatus').includes(data.status || status) && !this.get('suppressSource').includes(data.source)) {
      ...
      if(this.get('file')) {
        fs.promises.appendFile(Loader.resolvePath(this.get('file')), message + '\n').then(() => {
          if(this.get('showPayload') && data.payload) {
            fs.promises.appendFile(Loader.resolvePath(this.get('file')), '\t' + data.payload + '\n');
          }
        });
      }
      if(this.get('console')) {
