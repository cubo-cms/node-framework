...
export default class Session extends Class {
  ...
  constructor(data = { sessionId: Session.generateKey() }) {
    ...
    this.success(() => {
      ...
      Log.info({ message: `Session is created`, source: this.type, payload: { sessionId: this.get('sessionId') } });
    });
  }
  ...
  static cleanUp() {
    ...
    Log.info({ message: `Session cleans up ${counter} session(s)`, source: this.type });
  }
