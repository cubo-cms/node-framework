...
export default class Model extends Class {
  ...
  default = {
    ...
    page: 1,
    pageSize: 100,
    show: ['id', 'name', 'description', 'introText', 'text', 'blob', 'metadata', 'category', 'tags'],
    hide: ['_id','password'],
    sort: ['name', 'up']
  }
  ...
  method = {
    ...
    get: function() {
      return new Promise((resolve, reject) => {
        // Invoke the driver
        Driver.create(this.prepareData(), this).then((driver) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      if(method) {
        if(typeof this.method[method] === 'function') {
          ...
          reject(JResponse.respond('notAllowed', { header: { Allow: Object.keys(this.method).join(', ').toUpperCase() } }, this.type));
        }
      } else {
  ...
  prepareData() {
    ...
    data['dataDriver'] = this.get('driver');
